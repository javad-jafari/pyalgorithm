...
class Trip:
    ...
    all_cities = (
        'Arak', 'Ardabil', 'Urmia', 'Isfahan', 'Ahvaz', 
        'Ilam', 'Bojnord', 'Bandar Abbas', 'Bushehr', 
        'Birjand', 'Tabriz', 'Tehran', 'Khorramabad', 
        'Rasht', 'Zahedan', 'Zanjan', 'Sari', 'Semnan', 
        'Sanandaj', 'Shahr-e Kord', 'Shiraz', 'Qazvin', 
        'Qom', 'Karaj', 'Kermanshah', 'Gorgan', 'Mashhad', 
        'Hamadan', 'Yasuj', 'Yazd'
        )
    ...
    def __call__(self, *args, **kwds) :
        ...
        return self.train.weight_capacity - sum([i.load_weight for i in self.passengers])
...
class Passenger:
    ...
    def attend_trip(self, trip):
        remind_w = trip.train.weight_capacity - sum([i.load_weight for i in trip.passengers])

        if self.load_weight <= remind_w :
